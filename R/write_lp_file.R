#'\code{write_lp_file}
#'
#'Wting of the LP file
#'

write_lp_file <- function(dataGMM, 
                          pknList,
                          targets, 
                          experiments, 
                          nSolutions, 
  ...
  dataMatrix <- buildDataMatrix(dataGMM = dataGMM, pknList = pknList, 
                                targets = targets, experiments = experiments)
  ...
  binary_x <- 
    create_binary_variables_for_x_vector(dataMatrix = dataMatrix)
  binary_y <- 
    create_binary_variables_for_y_vector(pknList = pknList)
  binary_z <- 
    create_binary_variables_for_z_vector(pknList = pknList, 
                                         dataMatrix = dataMatrix)
  binary_orin_orout <- 
    create_orin_orout_variables(dataMatrix = dataMatrix, targets = targets, 
                                pknList = pknList)
  binaries <- 
    create_binaries(binaries_x = binary_x, binaries_z = binary_z, 
                    binaries_in_out = binary_orin_orout, binaries_y = binary_y)
  ...
  bounds <- 
    write_boundaries(binaries = binaries, pknList = pknList, M = 100, 
                     dataMatrix = dataMatrix)
  
  # Writing equality constraints
  eC <- 
    write_equality_constraints(dataMatrix = dataMatrix, binaries = binaries, 
                               pknList = pknList)
  
  # Writing Constraint - 1
  c1 <- 
    write_constraints_1(dataMatrix = dataMatrix, binaries = binaries, 
                        pknList = pknList)
  
  # Writing Constraint - 2
  c2 <- 
    write_constraints_2(dataMatrix = dataMatrix, binaries = binaries, 
                        pknList = pknList)
  
  # Writing Constraint - 3
  c3 <- 
    write_constraints_3(dataMatrix = dataMatrix, binaries = binaries, 
                        pknList = pknList)
  
  # Writing Constraint - 4
  c4 <- 
    write_constraints_4(dataMatrix = dataMatrix, binaries = binaries, 
                        pknList = pknList)
  
  # Writeing Constraint - 5
  c5 <- 
    write_constraints_5(dataMatrix = dataMatrix, binaries = binaries, 
                        pknList = pknList)
  
  # Writeing Constraint - 6
  c6 <- 
    write_self_activating_constraints(pknList = pknList, binaries = binaries, 
                                      dataMatrix = dataMatrix, M = 100)
  
  # Writeing Constraint - 7
  # c7 <- 
  #   write_in_out_constraints(binaries = binaries, targets = targets, 
  #                            dataMatrix = dataMatrix, pknList = pknList)
  
  # Putting all constraints together in one file
  # allC <- 
  #   all_constraints(equalityConstraints = eC, constraints1 = c1, 
  #                   constraints2 = c2, constraints3 = c3, constraints4 = c4, 
  #                   constraints5 = c5, constraints6 = c(c6, c7))
  allC <- 
    all_constraints(equalityConstraints = eC, constraints1 = c1, 
                    constraints2 = c2, constraints3 = c3, constraints4 = c4, 
                    constraints5 = c5, constraints6 = c6)
