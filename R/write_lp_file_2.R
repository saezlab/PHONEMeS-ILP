#'\code{write_lp_file_2}
#'
#'Wting of the LP file for the time-point variant - Other time-points besides
#'the first one
#'

write_lp_file_2 <- function(prevSIF, dataGMM, pknList, targets, experiments, penFac){
  ...
  c1 <- write_constraints_1(dataMatrix = dataMatrix, binaries = binaries, 
                            pknList = pknList)
  
  # Writing Constraint - 2
  c2 <- write_constraints_2(dataMatrix = dataMatrix, binaries = binaries, 
                            pknList = pknList)
  
  # Writing Constraint - 3
  c3 <- write_constraints_3(dataMatrix = dataMatrix, binaries = binaries, 
                            pknList = pknList)
  ...
  c6 <- write_time_point_constraints(binaries = binaries, tempSIF = prevSIF, 
