runPHONEMeS <- function(targets.P = targets.P, conditions = conditions, dataGMM = dataGMM, experiments = experiments, nK = nK, solver="cplex"){
  ...
  pknList<-buildNw(data.On=data.P, targets.On=targets.P, bg=bg, nK = nK)
  ...
  if(length(TG)==1){
    ...
    dataMatrix <- buildDataMatrix(dataGMM = dataGMM, pknList = pknList, targets = targets, experiments = conditions[experiments])
