build_PKN_Inv <-function(data.On,targets.On, bg,
  ...
  # head(bg@interactions)
  pSdf<-interactions(bg)
  pSdf <- pSdf[complete.cases(pSdf), ]
  ...
  if (sum(is.na(pSdf[,"K.ID"])) != 0 & sum( is.na(pSdf[,"S.ID"]) != 0))
  {
  ...
  if(length(allD.nw@nodes)==0){
    ...
    if(nK == "data"){
      ...
      nw.2use<-makeNetwork(source=pSdf[,"S.ID"], 
                           target=pSdf[,"K.ID"], 
                           edgemode="directed")
      nw.2use<-igraph.from.graphNEL(nw.2use)
      pSdf.2use<-pSdf
    }
    #link targetsPath.I[,"S.ID"] (or targetsPath.I[,"K.ID"]) to dataSites.I[,"K.ID"]
    dataKVn<-match(unique(dataSites.I[,"S.ID"]), V(nw.2use)$name) #modified from K.ID
    ...
    for(t in 1:length(dataKVn)){
      ...
      Paths<-get.all.shortest.paths(g=nw.2use, 
                                    ...
                                    mode="in", weights=NA)
      Paths<-lapply(Paths$res, function(x){return(V(nw.2use)$name[x])})
      # print(V(nw.2use)[dataKVn[t]]$name)
      # if(V(nw.2use)[dataKVn[t]]$name == "ARFP1_HUMAN")
      # {
      #   print(V(nw.2use)[dataKVn[t]])
      #   print(Paths)
      # }
    ...
    # print(data2targetPaths)
    #remove the initialiser line and the duplicates
    if(!is.null(dim(data2targetPaths)[1])){
      ...
      allD.2keep<-apply(data2targetPaths, MARGIN=1, 
                        function(x){k<-which(pSdf.2use[,"S.ID"] == x[1]); #modified from K.ID
                        s<-which(pSdf.2use[,"K.ID"] == x[2]); #modified from S.ID
                        # print(x[1]);
      ...
      #names(complete.I)[2] <- "S.ID"
      #names(complete.I)[4] <- "K.ID" #Modified
      #names(complete.I)[8] <- "S.cc" #Modified
      
      # print(head(complete.I))
      ...
      # complete.I <- complete.I[,c(1,2,3,8,5,6,7,4,9)] #Modified flip columns
      colnames(complete.I) <- colnames(complete.I)[c(1,2,3,8,5,6,7,4,9)] #Modified flip names
