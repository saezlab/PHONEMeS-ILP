...
write_equality_constraints <- function(dataMatrix = dataMatrix, binaries = binaries, pknList = pknList){
  ...
  for(ii in 1:nrow(sif)){
    ...
    for(jj in 1:nrow(dM)){
      ...
      ll[[ii]] = c(ll[[ii]], binaries[[1]][which(binaries[[3]]==paste0("interaction ", sif[ii, 1], "=", sif[ii, 3], " in experiment ", jj))])
      
    }
    
    ll[[ii]] = ll[[ii]][-1]
    names(ll)[ii] = binaries[[1]][which(binaries[[3]]==paste0("reaction ", sif[ii, 1], "=", sif[ii, 3]))]
...
write_in_out_constraints <- function(binaries = binaries, targets = targets, dataMatrix = dataMatrix, pknList = pknList){
  ...
  if((any(grepl(pattern = "orin", x = binaries[[3]]))) && (any(grepl(pattern = "orout", x = binaries[[3]])))){
