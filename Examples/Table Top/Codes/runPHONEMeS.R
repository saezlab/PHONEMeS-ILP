runPHONEMeS <- function(dataInput = dataInput, bg = bg, targets.P = targets.P, data.P = data.P, experiments = experiments){
  ...
  write.table(unique(resultsSIF1), file = "resultsSIF.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
  ...
  write.table(nodesAttributes, file = "nodesAttributes.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
  
  result <- list()
  result[[1]] <- resultsSIF
  result[[2]] <- nodesAttributes
  
  names(result) <- c("SIF", "nodesAttributes")
