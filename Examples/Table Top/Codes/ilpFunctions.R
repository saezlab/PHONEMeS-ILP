...
##
write_objective_function <- function(dataMatrix = dataMatrix, binaries = binaries, sizePen = sizePen, sizePenType = sizePenType, lambda = lambda){
  ...
  targets <- dataMatrix$species[dataMatrix$tgID]
  idxReac <- which(grepl(pattern = "reaction ", x = binaries[[3]]))
  reactions <- sapply(strsplit(x = binaries[[3]][idxReac], split = " "), "[[", 2)
  tgReacs <- sapply(strsplit(x = reactions, split = "="), "[[", 2)

  for(i in 1:length(targets)){

    idx <- which(tgReacs==targets[i])
    if(length(idx) > 0){

      for(j in 1:length(idx)){

        objectiveFunction <- paste0(objectiveFunction, " - 0.001 ", binaries[[1]][idxReac[idx[j]]])
        kk <- c(kk, binaries[[3]][idxReac[idx[j]]])
    ...
    if(sizePenType=="edge"){
      ...
      for(i in 1:length(binaries[[3]])){
        ...
        if(strsplit(binaries[[3]][i], split = " ")[[1]][1] == "reaction"){
          ...
          objectiveFunction <- paste(objectiveFunction, " + ", lambda, binaries[[1]][i], sep = "")
...
##
readOutSIF<- function(cplexSolutionFileName, binaries = binaries, pknList = pknList, dataMatrix = dataMatrix){
  ...
  pknSIF <- createSIF(pknList = pknList)
  ...
  if(length(targets) > 1){
    ...
    gg1 <- graph_from_data_frame(d = sif[, c(1, 3)], directed = TRUE)
    gg2 <- graph_from_data_frame(d = pknSIF[, c(1, 3)], directed = TRUE)
    
    adj1 <- get.adjacency(graph = gg1)
    adj2 <- get.adjacency(graph = gg2)
    ...
    for(i in 1:length(targets)){
      ...
      for(j in 1:length(targets)){
        ...
        if(i != j){
          ...
          sp1 <- get.shortest.paths(graph = gg1, from = which(rownames(adj1)==targets[i]), to = which(rownames(adj1)==targets[j]))
          
          if(length(sp1[[1]][[1]]) == 0){
            
            sp2 <- get.shortest.paths(graph = gg2, from = which(rownames(adj2)==targets[i]), to = which(rownames(adj2)==targets[j]))
            
            if((length(sp2[[1]][[1]]) > 0) && (cnt == 0)){
              
              for(ii in 1:(length(sp2[[1]][[1]])-1)){
                
                sif <- rbind(sif, c(rownames(adj2)[sp2[[1]][[1]][ii]], "1", rownames(adj2)[sp2[[1]][[1]][ii+1]]))
...
##
write_self_activating_constraints <- function(sif = sif, binaries = binaries, dataMatrix = dataMatrix, M = M){
