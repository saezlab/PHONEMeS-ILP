...
##############################################################################
# 12:58 23/03/2018
# Here are implemented the functions which write the objetive functions and ILP
# constraints of PHONEMeS. Note: Not all functions in this script are necessairly
# used to run PHONEMeS.
...
##
write_objective_function <- function(dataMatrix = dataMatrix, binaries = binaries, sizePen = TRUE, penMode = "node"){
    ...
    for(j in 1:ncol(dM)){
      if(as.numeric(dM[i, j]) <= 0){
        if(as.numeric(dM[i, j]) == 0){
          ...
          objectiveFunction <- paste(objectiveFunction, " ", 100*as.numeric(dM[i, j]), " ", "xb", kk, sep = "")
          kk <- kk+1
        }
      }
      else{
  ...
  ds <- gsub(pattern = "DS:", replacement = "", x = colnames(dataMatrix$dataMatrix)[dataMatrix$dsID])
    ...
    if(strsplit(binaries[[3]][i], split = " ")[[1]][1] == "reaction"){
      ...
      if(grepl(pattern = "_R1", x = r1) || grepl(pattern = "_R1", x = r2)){
        ...
        if(cc[1]==FALSE && cc[2]==TRUE){
          ...
          objectiveFunction <- paste(objectiveFunction, " - 0.1 ", binaries[[1]][i], sep = "")
        ...
        if(grepl(pattern = r2, x = r1)){
          ...
          objectiveFunction <- paste(objectiveFunction, " + 0.00001 ", binaries[[1]][i], sep = "")
...
# 
#           # if(strsplit(binaries[[3]][i], split = " ")[[1]][2]%in%pknList@interactions$S.cc[which(pknList@interactions$K.ID==dataMatrix$species[dataMatrix$tgID])]){
#           # 
#           #   objectiveFunction <- paste(objectiveFunction, " - 0.1 ", binaries[[1]][i], sep = "")
#           # 
#           # } else {
  ...
  for(i in 1:length(kinases)){
    ...
    if(length(idx) > 1){
      ...
      for(j in 1:length(idx)){
        ...
        if(length(strsplit(resultsSIF1[idx[j], 1], split = "_")[[1]]) > 2){
...
removeRedundantNodes <- function(resultsSIF1 = resultsSIF1){
  ...
  if(length(idxToRem) > 0){return(temp[-idxToRem, ])} else {return(temp)}
...
##
write_self_activating_constraints_2 <- function(pknList = pknList, binaries = binaries, dataMatrix = dataMatrix, M = M){
  ...
  idx <- which(grepl(pattern = binaries[[1]][which(grepl(pattern = "species PDGFR ", x = binaries[[3]]))], x = cc1))
  cc1 <- cc1[-idx]
  cc3 <- cc3[-idx]
  
  idx <- which(grepl(pattern = binaries[[1]][which(grepl(pattern = "species PDGFR ", x = binaries[[3]]))], x = cc2))
  cc2 <- cc2[-idx]
...
# ##
# write_objective_function_2 <- function(dataMatrix = dataMatrix, binaries = binaries, sizePen = TRUE, penMode = "node"){
# 
#   dM <- dataMatrix[[1]]
#   kk <- 1
#   objectiveFunction <- "obj:\t"
...
##
write_objective_function_2 <- function(dataMatrix = dataMatrix, binaries = binaries, sizePen = TRUE, penMode = "edge"){
